...
<head>
    ...
    if (isset($_REQUEST['btnrem'])) {
        $drid=$_POST["btnrem"];
        $removequery=mysqli_query($con,"DELETE FROM doctor WHERE drid='$drid'"); 
        header('refresh:0');
    }
    ?>
...
<body>
    <form action="" method="post">
        ...
        <button type='submit' class='btn btn-danger mt-1' value=" . $row['drid'] . " name='btnrem'>Remove</button>
        </div>
                </div>
                ";
            }
